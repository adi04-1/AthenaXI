  ...
  template: `
    <div class="admin-page">
      <div class="admin-page-header">
        <div>
          <h1 class="athena-page-title" style="margin-bottom:4px">Ì±• Player Upload</h1>
          <p class="athena-label">Select season ‚Üí download template ‚Üí upload data</p>
        </div>
      </div>
          ...
          <div class="step-header">
            ...
            <div>
              ...
              <div class="athena-label">
                @if (uploadDone()) { Upload complete ‚Äî locked for this season. }
                @else { Upload only once ‚Äî re-upload requires starting over. }
              </div>
            </div>
          </div>
          ...
          @if (!uploadDone()) {
            ...
            @if (selectedFile()) {
              <button class="athena-btn athena-btn-primary" style="width:100%;margin-top:12px"
                ...
                {{ saving() ? '‚öôÔ∏è Processing...' : '‚öîÔ∏è Upload & Process' }}
              </button>
            }
          } @else {
              ...
              @if (uploadResult().errors?.length > 0) {
                <div class="result-errors">
                  @for (e of uploadResult().errors; track e) {
                    <div class="err-line">‚ö†Ô∏è {{ e }}</div>
                  }
                </div>
              }
            </div>
...
export class PlayersAdminComponent implements OnInit {
  ...
  templateIcon() { return { FreshAuction:'Ìø¢', AuctionWithRetentions:'Ìø°', DirectAllocation:'Ì¥µ' }[this.selectedSeason()?.mode as string] ?? 'Ì≥ã'; }
  ...
  modeLabel(m: string) { return { FreshAuction:'Ìø¢ Fresh', AuctionWithRetentions:'Ìø° Retention', DirectAllocation:'Ì¥µ Direct' }[m] ?? m; }
