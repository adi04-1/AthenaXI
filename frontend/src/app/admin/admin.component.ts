    ...
    <div class="admin-shell">
      <aside class="admin-sidebar">
        <div class="sidebar-brand">
          <span class="sidebar-helm">‚öîÔ∏è</span>
          <div>
        ...
        <div class="sidebar-footer">
          <a routerLink="/" class="sidebar-nav-item" style="opacity:0.6">
            <span class="nav-icon">‚Üê</span><span class="nav-text">Back to App</span>
          </a>
          <button class="sidebar-nav-item logout-btn" (click)="auth.logout()">
            <span class="nav-icon">Ì∫™</span><span class="nav-text">Logout</span>
          </button>
        </div>
      </aside>

      <div class="admin-mobile-header">
        <span class="athena-brand" style="font-size:15px">‚öîÔ∏è Admin</span>
...
export class AdminComponent {
  navItems = [
    { route: 'seasons',       icon: 'Ì∑ìÔ∏è', label: 'Seasons' },
    { route: 'auction',       icon: 'Ì¥®', label: 'Auction Lobby' },
    { route: 'players',       icon: 'Ì±•', label: 'Player Upload' },
    { route: 'notifications', icon: 'Ì¥î', label: 'Notifications' },
  ];
